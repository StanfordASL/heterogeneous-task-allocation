...
flag_type_task_allocation = 4; % 1: Dumb Heuristic , 2: Task Assignment (1 step) Section 4, 3: Task Assignment (multi-step homogenenous) Section 5.A, 4: Task Assignment (multi-step heterogeneous) Section 6.A,
                ...
                for k=1:1:time_receding_horizon
                    ...
                    for g_x=[1 grid_num_y]
                        for g_y=1:1:grid_num_x
                            ...
                            fixed_task_grid_reward(this_bin,k) = fixed_task_grid_reward(this_bin,k) + probability_create_intruder*inactive_intruder*reward_checking_intruder/(2*grid_num_x + 2*grid_num_y - 4);
                        end
                    end
                ...
                for k=1:1:time_receding_horizon
                    ...
                    for g_x=[1 grid_num_y]
                        for g_y=1:1:grid_num_x
                            this_bin = (g_y-1)*grid_num_y + g_x;
                            for n=1:1:num_agents
                                fixed_task_grid_reward(this_bin,k,n) = fixed_task_grid_reward(this_bin,k,n) + probability_create_intruder*inactive_intruder*reward_checking_intruder/(2*grid_num_x + 2*grid_num_y - 4);
                            end
                        end
                    end
