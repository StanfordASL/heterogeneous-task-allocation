...
class abstract_distributed_task_allocation_subroutine(object):
    ...
    def __init__(self, network, rewards, agents, lagrangian, verbose=False, linear_program=True):
        ...
        @ param agents A list of length one containing the agent description.
                       The agent's description is a tuple with two entries, the agent's name and the initial location of the agent.
        @ param lagrangian A nested dictionary containing the Lagrangian of constraint 1m.  The dictionary has two keys.
                        The first key denotes the location of the task.
                        The second key denotes the time at which the task is performed.
                        lagrangian[location][time] is the dual of the constraint for task at location location and time time.
        ...
        assert len(agents) == 1, "ERROR: only one agent should be specified"
        for agent in agents:
...
class cplex_distributed_task_allocation_subroutine(abstract_distributed_task_allocation_subroutine):
    def initialize_problem(self):
        problem = cplex.Cplex()
