...
\end{array}}{\textrm{maximize}} & \sum_{k\in\{1,\ldots F\}}\left(\left(\sum_{j\in n_{\textrm{cell}}}\left(\sum_{q\in\{1,\ldots,Q\}}\left(R_{k}^{q}[j]\sum_{i\in n_{\textrm{cell}}}x_{k}^{q}[i,j]\right)\right)\right)\right.\nonumber \\
 & \qquad\qquad-\left.\left(\sum_{q\in\{1,\ldots,Q\}}\left(\sum_{i\in n_{\textrm{cell}}}\sum_{j\in n_{\textrm{cell}}}C_{k}^{q}[i,j]\thinspace x_{k}^{q}[i,j]\right)\right)\right)
\end{align}
