...
flag_type_task_allocation = 8; % 1: Dumb Heuristic , 2,6: Task Assignment (1 step) Section 4, 3,7: Task Assignment (multi-step homogenenous) Section 5.A, 4,8: Task Assignment (multi-step heterogeneous) Section 6.A, 9: Generate and visualize stored rand solution

reward_checking_intruder = 500;

reward_patrol_increment = 0;

time_receding_horizon = 10; % integer >= 1
...
flag_generate_visualize_store_rand_solution = 1; % Put 0 to generate variables array, Put 1 to generate solution in 'stored_rand_solution.mat', put 2 to visulaize this solution

t_final = 300; % Simulation Run Time Steps
...
switch flag_type_task_allocation
    ...
    case 9
        ...
        switch flag_generate_visualize_store_rand_solution
            
            case 0 % Store variables
            ...
            case 1 % Generate solution
                load('stored_rand_variables.mat');
            ...
            case 2 % visulaize solution
                load('stored_rand_solution.mat')
...
for t=1:1:t_final
        ...
        case 9
            ...
            switch flag_generate_visualize_store_rand_solution
                ...
                case 0 % Store variables
                    ...
                    if t == t_final
                        save('stored_rand_variables.mat','flag_intruder_active_array','bin_intruder_array','flag_intruder_type_array')
                    end
                ...
                case 1 % Generate solution
                    ...
                    agent_check_intruder_type_receding_horizon = zeros(max_num_intruder,time_receding_horizon);
                    agent_check_intruder_type_receding_horizon(:,1) = agent_check_intruder_type;
                    ...
                    if flag_patrol_intruder == 1
                        ...
                        for k=1:1:time_receding_horizon
                            if t+k <= t_final
                                ...
                                for ni=1:1:max_num_intruder
                                    if flag_intruder_active_array(ni,t+k) == 1
                                        
                                        if k>1
                                            agent_check_intruder_type_receding_horizon(ni,k) = agent_check_intruder_type_receding_horizon(ni,k-1);
                                        end
                                        
                                        if agent_check_intruder_type_receding_horizon(ni,k) == 0 || agent_check_intruder_type_receding_horizon(ni,k) == 2
                                            this_bin = bin_intruder_array(ni,t+k);
                                            fixed_task_grid_reward(this_bin,k) = fixed_task_grid_reward(this_bin,k) + reward_checking_intruder;
                                        end
                                    else
                                        agent_check_intruder_type_receding_horizon(ni,k) = 0;
                                    end
                                end
                    ...
                    pos_agent_array_omniscient(:,t) = new_pos_agent;
                    ...
                    if t == t_final
                        stored_rand_total_reward_available_array = total_reward_available_array;
                        stored_rand_total_reward_earned_array = total_reward_earned_array;
                        stored_rand_fraction_total_reward_earned_array = fraction_total_reward_earned_array;
                        stored_rand_cumulative_total_reward_available_array = cumulative_total_reward_available_array;
                        stored_rand_cumulative_total_reward_earned_array = cumulative_total_reward_earned_array;
                        stored_rand_fraction_cumulative_total_reward_earned_array = fraction_cumulative_total_reward_earned_array;
                        
                        save('stored_rand_solution.mat','pos_agent_array_omniscient','stored_rand_total_reward_available_array','stored_rand_total_reward_earned_array',...
                            'stored_rand_fraction_total_reward_earned_array','stored_rand_cumulative_total_reward_available_array','stored_rand_cumulative_total_reward_earned_array','stored_rand_fraction_cumulative_total_reward_earned_array')
                    end
    ...
    if flag_show_probability_cloud == 1
        switch flag_type_task_allocation
            ...
            case 9
                if flag_generate_visualize_store_rand_solution == 1
                    for k=1:1:time_receding_horizon
                        for grid_j=1:1:n_variables
                            this_grid_show_prob = probability_cloud(grid_j,k);
